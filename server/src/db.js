...
if (isProduction && !process.env.TURSO_DATABASE_URL) {
  console.warn('TURSO_DATABASE_URL is not set. Production writes will not be durable until Turso is connected.')
}

export const db = createClient({
  url: process.env.TURSO_DATABASE_URL || 'file:./server/data/veloura.db',
...
async function initialize() {
  ...
  if (count === 0) {
    await db.batch(seedProducts.map((p) => ({
